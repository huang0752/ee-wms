<template>
  <WmsWorkbenchShell
    title="出库作业台"
    description="按锁库、拣货、复核、确认组织销售出库流程，突出缺口、同步状态和作业进度。"
    eyebrow="SHIPPING OPERATIONS"
    :metrics="metrics"
    :metric-columns="5"
  >
    <template #actions>
    ...
    <ElTable v-loading="loading" :data="rows" row-key="id" height="calc(100vh - 390px)">
      <ElTableColumn prop="order_no" label="出库单号" min-width="160" fixed="left" />
      ...
      <ElTableColumn prop="customer_id" label="客户ID" width="90" />
    ...
    <ElDialog v-model="createVisible" title="新建出库单" width="820px">
        ...
        <ElRow :gutter="16">
          ...
          <ElCol :span="8">
            <ElFormItem label="客户ID">
              <ElInputNumber v-model="form.customer_id" :min="1" controls-position="right" />
            </ElFormItem>
          </ElCol>
          <ElCol :span="8">
    ...
    >
      <template #lines>
        <ElTable :data="lineRows" border>
          ...
          <ElTableColumn prop="shipped_qty" label="出库" width="90" />
...
const selectedRow = ref<WmsOutboundOrder | null>(null);
const form = reactive<WmsOutboundForm>({
...
const metrics = computed(() => [
  buildStaticMetric("出库单", rows.value.length, "ri:file-list-3-line", "primary", "张", "当前列表"),
  ...buildDocumentMetrics(rows.value, [
    ...
    { label: "已完成", statuses: ["confirmed"], icon: "ri:checkbox-circle-line" },
  ]),
]);
...
async function refreshData() {
  loading.value = true;
  try {
    const response = await WmsOutboundAPI.list({
      page_no: 1,
      page_size: 100,
...
async function openDetail(row: WmsOutboundOrder) {
