<template>
  <div class="wms-warning">
    <ElCard shadow="never">
      <div class="bar">
        <ElInputNumber v-model="warehouseId" :min="1" placeholder="仓库ID" />
        <ElButton :icon="Refresh" @click="refreshData" />
        <ElButton type="primary" @click="scan">扫描预警</ElButton>
      </div>
    </ElCard>

    <ElCard shadow="never">
      <ElTable v-loading="loading" :data="rows" row-key="id" height="calc(100vh - 300px)">
        <ElTableColumn prop="warning_no" label="预警编号" min-width="140" />
        <ElTableColumn prop="warning_type" label="类型" width="130" />
        <ElTableColumn prop="material_id" label="物料ID" width="100" />
        <ElTableColumn prop="current_qty" label="当前数" width="110" />
        <ElTableColumn prop="threshold_qty" label="阈值" width="110" />
        <ElTableColumn prop="status" label="状态" width="100" />
        <ElTableColumn label="操作" width="190">
          <template #default="{ row }">
            <ElButton text type="primary" @click="openAdvice(row)">智能建议</ElButton>
            <ElButton text type="primary" :disabled="row.status === 'closed'" @click="close(row)">关闭</ElButton>
          </template>
        </ElTableColumn>
      </ElTable>
    </ElCard>
...
async function refreshData() {
  loading.value = true;
  try {
    const response = await WmsWarningAPI.list({ page_no: 1, page_size: 100 });
