...
class SelfService:
    ...
    def _safe_filename(value: str) -> str:
        filename = re.sub(r"[\\/:*?\"<>|\s]+", "-", value.strip())
        return filename.strip("-") or "企业"

    @classmethod
    async def get_usage_certificate_preview(
        cls,
        auth: AuthSchema,
        tenant_id: int | None,
        ...
        企业名称来自租户主数据，系统名称/版本来自环境配置，不接受前端传入。
        """
        if not tenant_id:
        ...
        now = datetime.now()
        certificate_no = f"{settings.USAGE_CERT_NO_PREFIX}-{tenant.code}-{now:%Y%m%d-%H%M%S}"
        user = auth.get_user()
        ...
        coverage_scope = settings.USAGE_CERT_COVERAGE_SCOPE or "-"
        generated_at = f"{now:%Y年%m月%d日 %H:%M:%S}"
